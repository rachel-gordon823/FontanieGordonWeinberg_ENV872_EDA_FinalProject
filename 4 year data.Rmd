...
UkraineData <- read.csv("./Data/RawData/Ukraine_AQIData_4.csv", stringsAsFactors = TRUE)
...
Dnipro_2019 <- Ukraine_Processed %>% 
  filter(City %in% "Dnipro")%>%
  subset(date > "2019-2-28" & date < "2019-04-01") %>%
  mutate(Month = month(date), 
         Day = day(date), 
          Year = as.factor(year(date)))
write.csv(Dnipro_2019, row.names = FALSE, file = "./Data/Processed/Dnipro_2019.csv")

Dnipro_2020 <- Ukraine_Processed %>% 
  filter(City %in% "Dnipro")%>%
  subset(date > "2020-2-28" & date < "2020-04-01") %>%
  mutate(Month = month(date), 
         Day = day(date), 
          Year = as.factor(year(date)))
write.csv(Dnipro_2020, row.names = FALSE, file = "./Data/Processed/Dnipro_2020.csv")
...
Lviv_2019 <- Ukraine_Processed %>% 
  filter(City %in% "Lviv") %>%
  subset(date > "2019-2-28" & date < "2019-04-01") %>%
  mutate(Month = month(date), 
         Day = day(date), 
          Year = as.factor(year(date)))
write.csv(Lviv_2019, row.names = FALSE, file = "./Data/Processed/Lviv_2019.csv")

Lviv_2020 <- Ukraine_Processed %>% 
  filter(City %in% "Lviv") %>%
  subset(date > "2020-2-28" & date < "2020-04-01") %>%
  mutate(Month = month(date), 
         Day = day(date), 
          Year = as.factor(year(date)))
write.csv(Lviv_2020, row.names = FALSE, file = "./Data/Processed/Lviv_2020.csv")
...
FULL_DNIPRO <- bind_rows(Dnipro_2019, Dnipro_2020, Dnipro_2021,Dnipro_2022)
write.csv(FULL_DNIPRO, row.names = FALSE, file = "./Data/Processed/FULL_DNIPRO.csv")

FULL_LVIV <- bind_rows(Lviv_2019, Lviv_2020, Lviv_2021,Lviv_2022)
...
Full_air_quality_2019_2022 <- bind_rows(FULL_LVIV, FULL_DNIPRO)%>%
  mutate(Year = as.factor(Year))%>%
  mutate(City_Year=(paste(City,"-",Year)))



PM25_heatmap <- ggplot(Full_air_quality_2019_2022, aes(x = City_Year, y = pm25, fill = pm25))+
geom_tile() +
...
PM10_heatmap <- ggplot(Full_air_quality_2019_2022, aes(x = City_Year, y = pm10, fill = pm10))+
geom_tile() +
  scale_fill_gradient2(
  low = "#66FF00",
  mid = "#FFFF33",
...
PM25_dotplot <- ggplot(Full_air_quality_2019_2022, 
...
PM10_dotplot <- ggplot(Full_air_quality_2019_2022, 
